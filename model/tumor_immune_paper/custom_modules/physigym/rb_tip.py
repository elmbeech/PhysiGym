...
class ReplayBufferLatentSpace:
    ...
    def __init__(
        ...
        self.state = torch.empty(
            (self.buffer_size, *latent_dim), dtype=torch.float32, device=device
        )
        self.next_state = torch.empty(
            (self.buffer_size, *latent_dim), dtype=torch.float32, device=device
        )
        ...
        self.done = torch.empty((self.buffer_size, 1), dtype=torch.uint8, device=device)
